...
bool Inicializa(TipoApontador *Dicionario) { 
    *Dicionario = NULL; 
...
void Reconstitui(TipoApontador ApPag, TipoApontador ApPai, int PosPai,
                 short *Diminuiu) {
    TipoPagina *Aux;
    long DispAux, j;
...
void Antecessor(TipoApontador Ap, int Ind, TipoApontador ApPai,
                short *Diminuiu) {
...
void ImprimeI(TipoApontador p, int nivel) {
    ...
    for (i = 0; i < p->n; i++) printf("%ld ", (long)p->r[i].Chave);
    putchar('\n');
    nivel++;
